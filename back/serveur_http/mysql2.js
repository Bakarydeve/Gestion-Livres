...
async function modifierLivre(id, nouveauTitre, nouvelleAnnee) {
    return new Promise(async (resolve, reject) => {
        try {
            const pool = mysql2.createPool(config_mysql2);

            const sql = `UPDATE t_livre SET titre = ?, annee = ? WHERE id = ?`;
            const values = [nouveauTitre, nouvelleAnnee, id];

            const [result] = await pool.execute(sql, values);
            pool.end();

            if (result.affectedRows > 0) {
                resolve({ success: true, message: 'Livre modifié avec succès.' });
            } else {
                resolve({ success: false, message: 'Aucun livre trouvé avec cet identifiant.' });
            }
        } catch (error) {
            console.error('Erreur lors de la modification du livre :', error);
            reject({ success: false, message: 'Erreur lors de la modification du livre.' });
        }
    });
}
